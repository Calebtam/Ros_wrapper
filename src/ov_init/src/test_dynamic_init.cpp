...
//  std::shared_ptr<InertialInitializerOptions> params;
  auto params = std::make_shared<InertialInitializerOptions>();
  ...
  auto params_ptr = std::make_shared<ov_msckf::VioManagerOptions>();
  // Our initialization class objects
  auto imu_readings = std::make_shared<std::vector<ov_core::ImuData>>();
  auto tracker = std::make_shared<ov_core::TrackSIM>(params->camera_intrinsics, 0, params_ptr);
  ...
  while (sim.ok()) {
    ...
    bool hasimu = sim.get_next_imu(message_imu.timestamp, wm, am);
    sensor_msgs::ImuPtr imu(new sensor_msgs::Imu);
    imu->linear_acceleration.x = am(0);
    imu->linear_acceleration.y = am(1);
    imu->linear_acceleration.z = am(2);
    imu->angular_velocity.x = wm(0);
    imu->angular_velocity.y = wm(1);
    imu->angular_velocity.z = wm(2);
    message_imu.data = imu;
    if (hasimu) {
    ...
    if (hascam) {
      ...
      if (success) {
        ...
        tracker = std::make_shared<ov_core::TrackSIM>(params->camera_intrinsics, 0, params_ptr);
