...
bool DynamicInitializer::initialize(double &timestamp, Eigen::MatrixXd &covariance, std::vector<std::shared_ptr<ov_type::Type>> &order,
  ...
  {
    std::unique_lock<std::mutex> lck(*mtx_imu_data);
    readings = InitializerHelper::select_imu_readings(*imu_data, time0_in_imu, time1_in_imu);
  }
...
//    Eigen::Vector3d am = 0.5 * (imu0.am + imu1.am) - accelerometer_bias;
    Eigen::Vector3d wm;
    wm(0) = 0.5 * (imu0.data->angular_velocity.x + imu1.data->angular_velocity.x) - gyroscope_bias(0);
    wm(1) = 0.5 * (imu0.data->angular_velocity.y + imu1.data->angular_velocity.y) - gyroscope_bias(1);
    wm(2) = 0.5 * (imu0.data->angular_velocity.z + imu1.data->angular_velocity.z) - gyroscope_bias(2);
    Eigen::Vector3d am;
    am(0) = 0.5 * (imu0.data->linear_acceleration.x + imu1.data->linear_acceleration.x) - accelerometer_bias(0);
    am(1) = 0.5 * (imu0.data->linear_acceleration.y + imu1.data->linear_acceleration.y) - accelerometer_bias(1);
    am(2) = 0.5 * (imu0.data->linear_acceleration.z + imu1.data->linear_acceleration.z) - accelerometer_bias(2);
...
//      cpiI0toIi1->feed_IMU(imu0.timestamp, imu1.timestamp, imu0.wm, imu0.am, imu1.wm, imu1.am);
      cpiI0toIi1->feed_IMU(imu0.timestamp, imu1.timestamp, imu0.data, imu1.data);
    }
...
//      cpiIitoIi1->feed_IMU(imu0.timestamp, imu1.timestamp, imu0.wm, imu0.am, imu1.wm, imu1.am);
      cpiIitoIi1->feed_IMU(imu0.timestamp, imu1.timestamp, imu0.data, imu1.data);
    }
