...
public:
   ...
   */
  SimulatorInit(std::shared_ptr<InertialInitializerOptions> &params_);

  /**
   * @brief Will get a set of perturbed parameters
   * @param params_ Parameters we will perturb
   */
  void perturb_parameters(std::shared_ptr<InertialInitializerOptions> &params_);
  ...
  std::shared_ptr<InertialInitializerOptions> get_true_parameters() { return params; }
