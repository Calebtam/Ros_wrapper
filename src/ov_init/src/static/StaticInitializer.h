...
//#define DEBUG_STATIC_INITIALIZER
...
public:
   ...
   */
  explicit StaticInitializer(std::shared_ptr<InertialInitializerOptions> &params_, std::shared_ptr<ov_core::FeatureDatabase> db,
                             std::shared_ptr<std::vector<ov_core::ImuData>> imu_data_, std::shared_ptr<std::mutex>& mtx)
      : params(params_), _db(db), mtx_imu_data(mtx), imu_data(imu_data_) {
#ifdef DEBUG_STATIC_INITIALIZER
    if(!fout.is_open())
      fout.open("/home/minjian/projects/ros_ws/test/result/static_init.txt");
#endif
  }

  explicit StaticInitializer(std::shared_ptr<InertialInitializerOptions> &params_, std::shared_ptr<std::vector<ov_core::ImuData>> imu_data_, std::shared_ptr<std::mutex>& mtx)
      : params(params_), mtx_imu_data(mtx), imu_data(imu_data_) {
#ifdef DEBUG_STATIC_INITIALIZER
    if(!fout.is_open())
      fout.open("/home/minjian/projects/ros_ws/test/result/static_init.txt");
#endif
...
private:
  ...
  std::shared_ptr<InertialInitializerOptions> params;

  /// Feature tracker database with all features in it
  std::shared_ptr<ov_core::FeatureDatabase> _db;

  /// Our history of IMU messages (time, angular, linear)
  std::shared_ptr<std::mutex> mtx_imu_data;
