...
InertialInitializer::InertialInitializer(std::shared_ptr<InertialInitializerOptions> &params_, std::shared_ptr<ov_core::FeatureDatabase> db)
    : params(params_), _db(db) {

  // Vector of our IMU data
  imu_data = std::make_shared<std::vector<ov_core::ImuData>>();

  mtx_imu_data = std::make_shared<std::mutex>();
  // Create initializers
  init_static = std::make_shared<StaticInitializer>(params, _db, imu_data, mtx_imu_data);
  init_dynamic = std::make_shared<DynamicInitializer>(params, _db, imu_data, mtx_imu_data);
}
...
bool InertialInitializer::initialize(double &timestamp, Eigen::MatrixXd &covariance, std::vector<std::shared_ptr<ov_type::Type>> &order,
  ...
  if (params->init_max_disparity > 0) {
    ...
    init_static->fout << "disp: " << std::fixed << newest_cam_time << ", " << avg_disp0 << ", " << avg_disp1 << std::endl;
#endif
