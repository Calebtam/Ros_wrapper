...
#ifndef USE_ROS
ROS1Visualizer::ROS1Visualizer(std::shared_ptr<VioManager> app, std::shared_ptr<Simulator> sim)
    :_app(app), _sim(sim), thread_update_running(false) {
#else
ROS1Visualizer::ROS1Visualizer(std::shared_ptr<VioManager> app, std::shared_ptr<Simulator> sim, std::shared_ptr<ros::NodeHandle> nh)
  ...
  if (save_total_state) {
    ...
    // files we will open
    std::string filepath_est = "state_estimate.txt", filepath_std = "state_deviation.txt", filepath_gt = "state_groundtruth.txt";
#ifdef USE_ROS
...
#endif
  // bind the reset publish function
  if (_app) {
    _app->m_publishRestfun = std::bind(&ROS1Visualizer::callback_pub_reset, this, placeholders::_1);
    PRINT_INFO("Bind the publist reset function \n");
  }
  // image processing thread
  image_processing_thread = new std::thread(&ROS1Visualizer::image_processing, this);

  // viewer
#ifdef PC_VISUALIZATION
  viewer_ptr = std::make_shared<ov_visualization::SdViewer>(3);
  viewer_ptr->set_vo_reset_callback(std::bind(&ROS1Visualizer::reset_vo, this));
#endif
  vo_reset = false;
  /// frame rate calculation

  double frequency_diff = camera_frequency - _app->get_params()->track_frequency;
  if(frequency_diff < 1 )
  {
    interval_num = 1;
  }else{
    interval_num = camera_frequency / frequency_diff;
  }
}
...
#else
    odomIinM.header.stamp = ob_slam::Time(timestamp);
#endif
...
//  PRINT_DEBUG("callback_inertial 1\n");
  _app->feed_measurement_imu(message); 
//  PRINT_DEBUG("callback_inertial 2\n");
#ifdef USE_IMU_POSE
  visualize_odometry(message.timestamp);
#endif
//  PRINT_DEBUG("callback_inertial 3\n");
...
//    return;
  ...
  std::thread thread([&] {
    ...
    // Failure detection
    if (_app->if_detected_failure()) {
      PRINT_WARNING(RED "[TIME]: %.4f seconds set detected failure and start reset the VioManager\n", RESET, message.timestamp);
      reset_visualizer();
      PRINT_WARNING(RED "[TIME]: %.4f seconds set detected failure and end the reset\n" RESET, message.timestamp);
    }
    ...
    if (unique_cam_ids.size() == num_unique_cameras) {
      ...
      while (!camera_queue.empty() && camera_queue.at(0).timestamp < timestamp_imu_inC) {
        ...
        visualize();
        // std::cout << "pop msg: " << to_string(camera_queue.at(0).timestamp) << std::endl;
...
#else
//  PRINT_DEBUG("get imu:  %s\n", std::to_string(message.timestamp).c_str());
  std::lock_guard<std::mutex> lck(imu_camera_syn_mtx);
  latest_imu_msg_timestamp = message.timestamp;
...
//  }
  ...
  count_num++;
  if(count_num % interval_num == 0 && interval_num != 1) return;
  ...
  message.images.push_back(cv_ptr); // no need clone
...
//    PRINT_DEBUG("get image:  %s, %d\n", std::to_string(message.timestamp).c_str(), camera_queue.size());
...
void ROS1Visualizer::callback_stereo(const sensor_msgs::ImageConstPtr &msg0, const sensor_msgs::ImageConstPtr &msg1, int cam_id0,
  ...
  message.images.push_back(cv_ptr0);
  message.images.push_back(cv_ptr1);
...
void ROS1Visualizer::query_vio_status(){
  // here, send the vio status
  static boost::posix_time::ptime referTime = boost::posix_time::microsec_clock::local_time();
  static double init_window_time = _app->get_params()->init_options->init_window_time*
                                                _app->get_params()->state_transfer_rate*1000;
  if(_app->initialized()){
    current_vision_status = 2;
    return;
  }
  std::lock_guard<std::mutex> lck(vio_mutex);
  if(last_vio_status != vio_status)
    referTime = boost::posix_time::microsec_clock::local_time();
  // TODO: need to refactor
  if(vio_status == 0){
    last_vio_status = vio_status;
    boost::posix_time::ptime curTime = boost::posix_time::microsec_clock::local_time();
    double time_total = (curTime - referTime).total_milliseconds();
    if(time_total > init_window_time){
      last_vio_status = 0;
      vio_status = 1;
    }
  }else if(vio_status == 1 && !_app->initialized()){
    last_vio_status = vio_status;
  }
  current_vision_status = vio_status;
}
...
//  geometry_msgs::PoseWithCovarianceStamped poseIinM;
  nav_msgs::Odometry poseIinM;
#ifdef USE_ROS
  poseIinM.header.stamp = ros::Time(timestamp_inI);
#else
  poseIinM.header.stamp = ob_slam::Time(timestamp_inI);
#endif
  ...
  poseIinM.twist.twist.linear.x = state->_imu->vel()(0);
  poseIinM.twist.twist.linear.y = state->_imu->vel()(1);
  poseIinM.twist.twist.linear.z = state->_imu->vel()(2);
#ifdef USE_ROS
  pub_poseimu.publish(poseIinM);
#endif
  if(pose_callback_fun){
    auto pose_msg = std::make_shared<nav_msgs::Odometry>(poseIinM);
    pose_callback_fun(pose_msg);
  }
  if(odometry_distance_callback_fun){
    odometry_distance_callback_fun(_app->get_vio_distance());
  }
  ...
  for (size_t i = 0; i < poses_imu.size(); i += std::floor((double)poses_imu.size() / 65536.0) + 1) {
  ...
  poses_seq_imu++;
  //feed path to viewer, do not publish here
#ifdef PC_VISUALIZATION_0
  viewer_ptr->SetCameraPose(poseIinM);
//  std::cout << "pose: " << arrIMU.poses.size() << ", " << poseIinM.pose.pose.position.x << ", " << poseIinM.pose.pose.position.y << ", "
//            << poseIinM.pose.pose.position.z << std::endl;
  if(poses_seq_imu %1 == 0)
    viewer_ptr->SetKeyframes(arrIMU);
#endif
}
...
void ROS1Visualizer::publish_images() {
  ...
  if(_app->get_params()->display_image) {
#ifdef DISPLAY_IMAGE_TRACK
    // Get our image of history tracks
    cv::Mat img_history = _app->get_historical_viz_image();//WAR:high time consuming! 3~5ms in PC
    ...
    header.stamp.fromSec(last_visualization_timestamp_image);
...
#else
#ifdef DISPLAY_IMAGE_TRACK
    namedWindow("track", cv::WINDOW_NORMAL);
    cv::resize(img_history,img_history,cv::Size(img_history.cols/2,img_history.rows/2));
    cv::imshow("track", img_history);
    cv::waitKey(1);
#endif
#endif
  }
...
void ROS1Visualizer::reset_visualizer() {
  auto params = _app->get_params();
  _app.reset(new VioManager(params));
  // bind the reset publish function
  if (_app) {
    _app->m_publishRestfun = std::bind(&ROS1Visualizer::callback_pub_reset, this, placeholders::_1);
    PRINT_INFO("ReBind the publist reset function\n");
  }
  poses_imu.clear();
  camera_queue.clear();
  ...
  last_visualization_timestamp_image = 0;

  // for fusion
  if(vo_reset_callback_fun){
    vo_reset_callback_fun();
  }
  vo_reset = false;
}
//#include <sys/types.h>
...
#ifdef HPC_SETAFFINITY_OPT
//  pid_t tid = gettid();//available in x3m
//  pid_t tid = syscall(SYS_gettid);//available in PC
  cpu_set_t cpuset;//for x3m 0~3, for 6490 4~7
  CPU_ZERO(&cpuset);
  CPU_SET(6,&cpuset);
  CPU_SET(7,&cpuset);
//  int rc = sched_setaffinity(tid, sizeof(cpu_set_t), &cpuset);
  ...
  if (rc != 0) {
    PRINT_WARNING(RED "Error calling sched_setaffinity: %d\n" RESET, rc);
  }
#endif
  const auto params = _app->get_params();
  const size_t num_unique_cameras = (params->state_options.num_cameras == 2) ? 1 : params->state_options.num_cameras;
//  double lastImageStamp = 0.0;
//  bool isFirstRecordTime = true;
  while(m_processImgFlag) {
    thread_update_running = true;
    double latest_imu_msg_timestamp_cpy = -1;
    {
      std::lock_guard<std::mutex> lck(imu_camera_syn_mtx);
      latest_imu_msg_timestamp_cpy = latest_imu_msg_timestamp;
    }
    if(latest_imu_msg_timestamp_cpy > 0){
      // Failure detection
      if (_app->if_detected_failure() || vo_reset) {
        PRINT_WARNING(RED "[TIME]: %.4f seconds set detected failure and start reset the VioManager\n", RESET, latest_imu_msg_timestamp_cpy);
        reset_visualizer();
        PRINT_WARNING(RED "[TIME]: %.4f seconds set detected failure and end the reset\n" RESET, latest_imu_msg_timestamp_cpy);
        std::lock_guard<std::mutex> lck(vio_mutex);
        vio_status = 0;
      ...
      // Lock on the queue (prevents new images from appending)
      std::unique_lock<std::mutex> lck(camera_queue_mtx);//todo: lock range is too large!
      // Count how many unique image streams
      std::map<int, bool> unique_cam_ids;
      for (const auto &cam_msg : camera_queue) {
        unique_cam_ids[cam_msg.sensor_ids.at(0)] = true;
      }
//      PRINT_DEBUG("camera_queue: %s, %d\n", std::to_string(latest_imu_msg_timestamp_cpy).c_str(), camera_queue.size());
      // If we do not have enough unique cameras then we need to wait
      // We should wait till we have one of each camera to ensure we propagate in the correct order
      if (unique_cam_ids.size() == num_unique_cameras) {
        // Loop through our queue and see if we are able to process any of our camera measurements
        // We are able to process if we have at least one IMU measurement greater than the camera time
        double timestamp_imu_inC = latest_imu_msg_timestamp_cpy - _app->get_state()->_calib_dt_CAMtoIMU->value()(0);
        std::deque<ov_core::CameraData> camera_queue_temp;
        while(!camera_queue.empty() && camera_queue.at(0).timestamp < timestamp_imu_inC)
        {
          camera_queue_temp.push_back(camera_queue.at(0));
          camera_queue.pop_front();
        }
        lck.unlock();
        while (!camera_queue_temp.empty()){
#define TIME_STATISTICS_HZ
#ifdef TIME_STATISTICS_HZ
          auto rT0_1 = boost::posix_time::microsec_clock::local_time();
          double update_dt = 1000.0 * (timestamp_imu_inC - camera_queue_temp.at(0).timestamp);
#endif
          PRINT_INFO(BLUE "## camera_queue size: %d \n" RESET, camera_queue_temp.size());
#ifdef NarrowLockField
          ov_core::CameraData imageData = camera_queue_temp.at(0);
          camera_queue_temp.pop_front();
          _app->feed_measurement_camera(imageData);
#else
          _app->feed_measurement_camera(camera_queue.at(0));
#endif
          query_vio_status();
          if(vo_status_callback_fun)
            vo_status_callback_fun(current_vision_status);
          visualize();
#ifdef TIME_STATISTICS_HZ
          auto rT0_2 = boost::posix_time::microsec_clock::local_time();
          double time_total = (rT0_2 - rT0_1).total_microseconds() * 1e-6;
...
#endif
          PRINT_INFO(BLUE "## camera_queue size: %d \n" RESET, camera_queue.size());
//          PRINT_DEBUG("process msg: %s, %d\n", std::to_string(camera_queue.at(0).timestamp).c_str(), camera_queue.size());
//#define NarrowLockField
#ifdef NarrowLockField
          ov_core::CameraData imageData = camera_queue.at(0);
//          if(!isFirstRecordTime){    /// slam image time stamp diff
//            double diff = (imageData.timestamp - lastImageStamp)*1000;
//            if(diff > 55)
//              PRINT_INFO(RED "## slam camera stamp diff: %.5f \n" RESET, diff);  /// %.5f
//          } else
//            isFirstRecordTime = false;
//          lastImageStamp = imageData.timestamp;
          camera_queue.pop_front();
          lck.unlock();
          _app->feed_measurement_camera(imageData);
#else
          ...
          PRINT_INFO(BLUE "[TIME]: %.4f seconds total (%.1f hz, %.2f ms behind)\n" RESET, time_total, 1.0 / time_total, update_dt);
#endif
#ifndef NarrowLockField
          camera_queue.pop_front();
#else
          lck.lock();
#endif
        }   */
//        lck.unlock();   /// the unique will unlock automatically
#endif
      }
    }
    thread_update_running = false;
    if(!m_processImgFlag) { break;}
    unique_lock<mutex> lock(image_processing_thread_mtx);
    printf(RED " vio thread: wait for image !!\n" RESET);
    image_processing_thread_cv.wait(lock);
  }
...
//  } else {
//    image_processing_thread->detach();
//  }
}