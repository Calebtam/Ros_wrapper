...
class Propagator;
//typedef std::function<void(const geometry_msgs::PoseWithCovarianceStampedConstPtr&)> pose_msg_callback_fun_ptr;
typedef std::function<void(const nav_msgs::OdometryConstPtr&)> pose_msg_callback_fun_ptr;
/**
...
public:
   ...
   */
  VioManager(std::shared_ptr<VioManagerOptions> &params_);
  ...
  /// If detected failure
  bool if_detected_failure() { return detected_failure; }
  ...
  std::shared_ptr<VioManagerOptions> get_params() { return params; }
  ...
  double get_vio_distance();
  void publish_init_state();

public:
  std::function<void(bool)> m_publishRestfun; 
...
protected:
  ...
  /// Manager parameters
  std::shared_ptr<VioManagerOptions> params;
  ...
  // Feature statistic file
#ifdef FEATURE_STATISTICS
  std::ofstream of_feature_statistics;
#endif
  ...
  double last_no_slam_feature_time = 0.0;

public:
  // for publish of init state
