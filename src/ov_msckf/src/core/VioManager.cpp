...
VioManager::VioManager(std::shared_ptr<VioManagerOptions> &params_) : thread_init_running(false), thread_init_success(false) {
  ...
  if (params->record_feature_information) {
    if (boost::filesystem::exists(params->record_feature_filepath)) {
      boost::filesystem::remove(params->record_feature_filepath);
      PRINT_INFO(YELLOW "[STATS]: found old file found, deleted...\n" RESET);
    }
    // Create the directory that we will open the file in
    boost::filesystem::path p(params->record_feature_filepath);
    boost::filesystem::create_directories(p.parent_path());
    // Open our statistics file!
    of_feature_statistics.open(params->record_feature_filepath, std::ofstream::out | std::ofstream::app);
    // Write the header information into it
    of_feature_statistics << "# left_feature_num = stereo_feature_num + mono_left_feature_num" << std::endl;
    of_feature_statistics << "# right_feature_num = stereo_feature_num + mono_right_feature_num" << std::endl;
    of_feature_statistics << "# timestamp (sec), image id, ";
    of_feature_statistics << "stereo feature num, mono left feature num, mono right feature num" << std::endl;
  }
#endif
  //===================================================================================
  //===================================================================================
  ...
  if (params->use_klt) {
    trackFEATS = std::shared_ptr<TrackBase>(
        new TrackKLT(state->_cam_intrinsics_cameras, init_max_features, state->_options.max_aruco_features, params));
  } else {
    trackFEATS = std::shared_ptr<TrackBase>(
        new TrackDescriptor(state->_cam_intrinsics_cameras, init_max_features, state->_options.max_aruco_features, params));
  }

  // Initialize our aruco tag extractor
  if (params->use_aruco) {
    trackARUCO = std::shared_ptr<TrackBase>(new TrackAruco(state->_cam_intrinsics_cameras, state->_options.max_aruco_features, params));
  }
...
void VioManager::feed_measurement_simulation(double timestamp, const std::vector<int> &camids,
  ...
  if (trackSIM == nullptr) {
    ...
    trackSIM = std::make_shared<TrackSIM>(state->_cam_intrinsics_cameras, state->_options.max_aruco_features, params);
    trackFEATS = trackSIM;
  ...
  for (auto const &camid : camids) {
    ...
    cv_ptr->image = cv::Mat::zeros(cv::Size(width, height), CV_8UC1);
    message.images.push_back(cv_ptr);
...
void VioManager::track_image_and_update(const ov_core::CameraData &message_const) {
  ...
  for (size_t i = 0; i < message.sensor_ids.size() && params->downsample_cameras; i++) {
    cv::Mat img = message.images[i]->image;
    //    cv::Mat mask = message.masks.at(i);
    cv::Mat img_temp, mask_temp;
    cv::pyrDown(img, img_temp, cv::Size(img.cols / 2.0, img.rows / 2.0));
    //    message.images[i]->image = img_temp;//todo:not support!
...
void VioManager::do_feature_propagate_update(const ov_core::CameraData &message) {
  ...
  for (std::pair<const size_t, std::shared_ptr<Landmark>> &landmark : state->_features_SLAM) {
    ...
    if (feat2 == nullptr && current_unique_cam) {
      PRINT_DEBUG("should_marg-0: %d\n", landmark.first);
      landmark.second->should_marg = true;
    }
    if (landmark.second->update_fail_count > 1) {
      PRINT_DEBUG("should_marg-1: %d\n", landmark.first);
      landmark.second->should_marg = true;
    }
  }
  ...
  PRINT_INFO(YELLOW "q_GtoI = %.3f,%.3f,%.3f,%.3f | p_IinG = %.3f,%.3f,%.3f | dist = %.2f (meters)\n", state->_imu->quat()(0),
  ...
  PRINT_INFO("slam feature num: %d \n", state->_features_SLAM.size());
  // Debug for camera imu offset
  if (state->_options.do_calib_camera_timeoffset) {
    PRINT_INFO("state time = %.5f\n", state->_timestamp);
  ...
  if (thread_init_success) {
    if (state->_features_SLAM.size() == 0) {
      if (state->_timestamp - last_no_slam_feature_time > 2) {
        detected_failure = true;
        PRINT_WARNING(RED "[TIME]: %.4f seconds detected failure\n" RESET, state->_timestamp);
        if (m_publishRestfun)
          m_publishRestfun(true);
        //        exit(0);
      }
    } else {
      //      detected_failure = false;
      last_no_slam_feature_time = state->_timestamp;
    }
  }
}

/*
The reset function for the vio states
*/
// void VioManager::reset() {
//   printf("*************************************start reset thread.\n");
//   int init_max_features = std::floor((double)params->init_options.init_max_features / (double)params->state_options.num_cameras);
//   state.reset(new State(params->state_options));
//   trackFEATS.reset(new TrackKLT(state->_cam_intrinsics_cameras, init_max_features, state->_options.max_aruco_features,
//   params->use_stereo,
...
//                                   params->min_px_dist));
//   } else {
//     trackFEATS.reset(new TrackDescriptor(state->_cam_intrinsics_cameras, init_max_features, state->_options.max_aruco_features,
...
//   }
//   trackARUCO.reset(new TrackAruco(state->_cam_intrinsics_cameras, state->_options.max_aruco_features, params->use_stereo,
//                                   params->histogram_method, params->downsize_aruco));
//   propagator.reset(new Propagator(params->imu_noises, params->gravity_mag));
//   initializer.reset(new ov_init::InertialInitializer(params->init_options, trackFEATS->get_feature_database()));
//   updaterMSCKF.reset(new UpdaterMSCKF(params->msckf_options, params->featinit_options));
//   updaterSLAM.reset(new UpdaterSLAM(params->slam_options, params->aruco_options, params->featinit_options));
//   updaterZUPT.reset(new UpdaterZeroVelocity(params->zupt_options, params->imu_noises, trackFEATS->get_feature_database(), propagator,
//                                             params->gravity_mag, params->zupt_max_velocity, params->zupt_noise_multiplier,
