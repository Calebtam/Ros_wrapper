...
bool VioManager::try_to_initialize(const ov_core::CameraData &message) {
  ...
  std::thread thread([&] {
    ...
    if (success) {
      ...
      std::lock_guard<std::mutex> lck(camera_queue_init_mtx);
//      if(camera_queue_init.size() < 3)
        camera_queue_init.clear();
//      else{
//        // delete only some oldest measurements
//        double time_start = camera_queue_init.at(0) + (init_rT2 - init_rT1).total_microseconds() * 1e-6;
...
void VioManager::retriangulate_active_tracks(const ov_core::CameraData &message) {
  ...
  if (!active_image.empty()) {
    active_image = active_image(cv::Rect(0, 0, message.images[0]->image.cols, message.images[0]->image.rows));
  }
#endif
...
double VioManager::get_vio_distance(){
  return distance;
}
...
void VioManager::publish_init_state() {
...
//  geometry_msgs::PoseWithCovarianceStamped poseIinM;
  ...
  Eigen::Matrix<double, 6, 6> covariance_posori = StateHelper::get_marginal_covariance(state, statevars);
  for (int r = 0; r < 6; r++) {
    for (int c = 0; c < 6; c++) {
