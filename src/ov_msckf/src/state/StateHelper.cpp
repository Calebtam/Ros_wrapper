...
void StateHelper::EKFUpdate(std::shared_ptr<State> state, const std::vector<std::shared_ptr<Type>> &H_order, const Eigen::MatrixXd &H,
  ...
  Eigen::MatrixXd M_a = Eigen::MatrixXd::Zero(state->_Cov.rows(), res.rows());//P*H^T
  ...
  //==========================================================
  //==========================================================
//   std::cout << "var: " << state->_variables.size() << ", " << H_order.size() << std::endl;
  ...
  for (const auto &var : state->_variables) {
    ...
    const int v_id = var->id();
    const int v_size = var->size();
    for (size_t i = 0; i < H_order.size(); ++i) {
      std::shared_ptr<Type> meas_var = H_order[i];
      M_a.block(v_id, 0, v_size, res.rows()) += state->_Cov.block(v_id, meas_var->id(), v_size, meas_var->size()) *
          H_t.block(H_id[i], 0, meas_var->size(), H.rows());
    }
  }
#else
//  Eigen::MatrixXd M_b = Eigen::MatrixXd::Zero(state->_Cov.rows(), res.rows());
  const int Hrows = H.rows();
  const int Crows = state->_Cov.rows();
//  {
//#pragma omp parallel for num_threads(2)
//    for (size_t i = 0; i < H_order.size(); ++i) {
//      std::shared_ptr<Type> meas_var = H_order[i];
//      const int m_size = meas_var->size();
//      const int m_id = meas_var->id();
//      M_b += state->_Cov.block(0, m_id, Crows, m_size) * H_t.block(H_id[i], 0, m_size, Hrows);
//    }
//  }
  if(H_order.size() > 20) {
    std::vector<Eigen::MatrixXd> M_bs(H_order.size());
    parallel_for_(cv::Range(0, (int)H_order.size()), LambdaBody([&](const cv::Range &range) {
      for (int i = range.start; i < range.end; ++i) {
        M_bs[i] = Eigen::MatrixXd::Zero(state->_Cov.rows(), res.rows());
        std::shared_ptr<Type> meas_var = H_order[i];
        const int m_size = meas_var->size();
        const int m_id = meas_var->id();//m_id is out of order
        M_bs[i] = state->_Cov.block(0, m_id, Crows, m_size) * H_t.block(H_id[i], 0, m_size, Hrows);//H_id is ordered
      }
    }));
    ...
    for (size_t i = 0; i < H_order.size(); ++i) {
      ...
      const int m_size = meas_var->size();
      const int m_id = meas_var->id();//m_id is out of order
      M_a += state->_Cov.block(0, m_id, Crows, m_size) * H_t.block(H_id[i], 0, m_size, Hrows);//H_id is ordered
    }
  }
//  std::cout << "same? " << M_a.isApprox(M_b, 0.01) << std::endl;
#endif
#ifdef TIME_STATISTICS
  rT2 = boost::posix_time::microsec_clock::local_time();
//  PRINT_ALL("[EKFUpdate]: %.5f seconds to solu1\n", (rT1_1 - rT1).total_microseconds() * 1e-6);//0.0029ms
//  PRINT_ALL("[EKFUpdate]: %.5f seconds to solu2\n", (rT2 - rT1_1).total_microseconds() * 1e-6);//0.0011ms
//  static int cnt = 0;
//  if((rT2 - rT1_1) < (rT1_1 - rT1) && (rT1_1 - rT1).total_microseconds() * 1e-6 > 0.001){
//    PRINT_ERROR(RED "slow!! %d\n" RESET, ++cnt);
//  }
#endif
  //==========================================================
  //==========================================================
...
//  S.triangularView<Eigen::Upper>() = H * P_small * H.transpose();
  S.triangularView<Eigen::Upper>() = H * P_small * H_t;
...
//  auto rT2_1 = boost::posix_time::microsec_clock::local_time();
  S.selfadjointView<Eigen::Upper>().llt().solveInPlace(Sinv);//costless, 0.2ms
//  auto rT2_2 = boost::posix_time::microsec_clock::local_time();
  Eigen::MatrixXd K = M_a * Sinv.selfadjointView<Eigen::Upper>();
  // Eigen::MatrixXd K = M_a * S.inverse();

  // Update Covariance
  state->_Cov.triangularView<Eigen::Upper>() -= K * M_a.transpose();//time cost!
...
#endif
//  PRINT_ALL("[EKFUpdate]: %.4f seconds to llt0\n", (rT2_1 - rT2).total_microseconds() * 1e-6);//0.1ms
//  PRINT_ALL("[EKFUpdate]: %.4f seconds to llt1\n", (rT2_2 - rT2_1).total_microseconds() * 1e-6);//0.0ms
//  PRINT_ALL("[EKFUpdate]: %.4f seconds to llt2\n", (rT3 - rT2_2).total_microseconds() * 1e-6);//0.3ms
  // We should check if we are not positive semi-definitate (i.e. negative diagionals is not s.p.d)
  Eigen::VectorXd diags = state->_Cov.diagonal();
  bool found_neg = false;
  for (int i = 0; i < diags.rows(); i++) {
  ...
  PRINT_ALL("[EKFUpdate]: %.4f seconds to prepare\n", (rT1 - rT0).total_microseconds() * 1e-6);
  PRINT_ALL("[EKFUpdate]: %.4f seconds to calculate\n", (rT2 - rT1).total_microseconds() * 1e-6);
  PRINT_ALL("[EKFUpdate]: %.4f seconds solve\n", (rT3 - rT2).total_microseconds() * 1e-6);
  PRINT_ALL("[EKFUpdate]: %.4f seconds total\n", (rT4 - rT1).total_microseconds() * 1e-6);
#endif
}
...
Eigen::MatrixXd StateHelper::get_marginal_covariance(std::shared_ptr<State> state,
  ...
  for (size_t i = 0; i < small_variables.size(); i++) {
    int k_index = 0;
    const int m_id = small_variables[i]->id();
    const int m_size = small_variables[i]->size();
    for (size_t k = 0; k < small_variables.size(); k++) {
      const int k_id = small_variables[k]->id();
      const int k_size = small_variables[k]->size();
      Small_cov.block(i_index, k_index, m_size, k_size) =
          state->_Cov.block(m_id, k_id, m_size, k_size);
      k_index += k_size;
    }
    i_index += m_size;
  }
#if 0
  i_index = 0;
  Eigen::MatrixXd cov = Eigen::MatrixXd::Zero(cov_size, cov_size);
  for (size_t i = 0; i < small_variables.size(); i++) {
    const int m_id = small_variables[i]->id();
    const int m_size = small_variables[i]->size();
    cov.block(i_index, 0, m_size, cov_size) = state->_Cov.block(m_id, 0, m_size, cov_size);
    i_index += small_variables[i]->size();
  }
  std::cout << "same? " << Small_cov.isApprox(cov, 0.0001) << std::endl;
#endif
...
Eigen::MatrixXd StateHelper::get_full_covariance(std::shared_ptr<State> state) {
  ...
  full_cov.block(0, 0, cov_size, cov_size) = state->_Cov;
...
bool StateHelper::initialize(std::shared_ptr<State> state, std::shared_ptr<Type> new_variable,
  ...
  for (int n = 0; n < H_L.cols(); ++n) {
    for (int m = (int)H_L.rows() - 1; m > n; m--) {
      ...
      const auto tempHo_GR_adj = tempHo_GR.adjoint();
      (H_L.block(m - 1, n, 2, H_L.cols() - n)).applyOnTheLeft(0, 1, tempHo_GR_adj);
      (res.block<2, 1>(m - 1, 0)).applyOnTheLeft(0, 1, tempHo_GR_adj);
      (H_R.block(m - 1, 0, 2, H_R.cols())).applyOnTheLeft(0, 1, tempHo_GR_adj);
    }
  }
...
#else
  const int Hrows = H_R.rows();
  const int Crows = state->_Cov.rows();
//  Eigen::MatrixXd M_b = Eigen::MatrixXd::Zero(Crows, res.rows());
  for (size_t i = 0; i < H_order.size(); ++i) {
    std::shared_ptr<Type> meas_var = H_order[i];
    const int m_size = meas_var->size();
    const int m_id = meas_var->id();//m_id is out of order
...
//  std::cout << "same? " << M_a.isApprox(M_b, 0.0001) << std::endl;
//  PRINT_ALL("[initialize_invertible]: %.4f seconds to calculate1\n", (rT2 - rT1).total_microseconds() * 1e-6);
//  PRINT_ALL("[initialize_invertible]: %.4f seconds to calculate2\n", (rT3 - rT2).total_microseconds() * 1e-6);
#endif
  //==========================================================
  //==========================================================
  ...
  // M = H_R*Cov*H_R' + R
  Eigen::MatrixXd M(Hrows, Hrows);
  M.triangularView<Eigen::Upper>() = H_R * P_small * H_t;
  M.triangularView<Eigen::Upper>() += R;

  const int new_var_size = new_variable->size();
  // Covariance of the variable/landmark that will be initialized
  assert(H_L.rows() == H_L.cols());
  assert(H_L.rows() == new_var_size);
  ...
  state->_Cov.conservativeResizeLike(Eigen::MatrixXd::Zero(oldSize + new_var_size, oldSize + new_var_size));
  state->_Cov.block(0, oldSize, oldSize, new_var_size).noalias() = -M_a * H_Linv.transpose();
  state->_Cov.block(oldSize, 0, new_var_size, oldSize) = state->_Cov.block(0, oldSize, oldSize, new_var_size).transpose();
  state->_Cov.block(oldSize, oldSize, new_var_size, new_var_size) = P_LL;
...
void StateHelper::augment_clone(std::shared_ptr<State> state, Eigen::Matrix<double, 3, 1> last_w) {
  ...
  if (state->_options.do_calib_camera_timeoffset) {
    ...
    dnc_dt.block<3, 1>(0, 0) = last_w;
    dnc_dt.block<3, 1>(3, 0) = state->_imu->vel();
...
void StateHelper::marginalize_slam(std::shared_ptr<State> state) {
  ...
  PRINT_DEBUG(GREEN "marginalize_slam: %d \n" RESET, ct_marginalized);
}

void StateHelper::marginalize_keyframes(const std::shared_ptr<ov_core::FeatureDatabase>& databse, std::shared_ptr<State> state) {
  if ((int)state->_keyframes_IMU.size() > state->_options.max_keyframe_size) {
    std::vector<double> marginal_time = state->marg_keyframe_timestep(databse);
    for(size_t i = 0; i < marginal_time.size(); ++i)
    {
      assert(marginal_time[i] != INFINITY);
      StateHelper::marginalize(state, state->_keyframes_IMU.at(marginal_time[i]));
      // Note that the marginalizer should have already deleted the clone
      // Thus we just need to remove the pointer to it from our state
      state->_keyframes_IMU.erase(marginal_time[i]);
    }
  }
}

void StateHelper::marginalize_map(std::shared_ptr<State> state) {
  // Remove Map features that have their marginalization flag set
  // We also check that we do not remove any aruoctag landmarks
  int ct_marginalized = 0;
  auto it0 = state->_features_Map.begin();
  while (it0 != state->_features_Map.end()) {
    // see TrackBase() for this condition
    if ((*it0).second->should_marg && (int)(*it0).first > 4 * state->_options.max_aruco_features) {
      StateHelper::marginalize(state, (*it0).second);
      it0 = state->_features_Map.erase(it0);
      ct_marginalized++;
    } else {
      it0++;
    }
