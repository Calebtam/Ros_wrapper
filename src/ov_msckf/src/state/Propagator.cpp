...
void Propagator::propagate_and_clone(std::shared_ptr<State> state, double timestamp) {
  ...
  Eigen::Matrix<double, 15, 15> Qd_summed = MatZero15;
  double dt_summed = 0;

  // Loop through all IMU messages, and use them to move the state forward in time
  // This uses the zero'th order quat, and then constant acceleration discrete
  Eigen::Matrix<double, 15, 15> F, Qdi;
  printf(RED "STEP 2: " RESET);
  if (prop_data.size() > 1) {
  ...
  printf(RED "STEP 3: " RESET);
  Eigen::Matrix<double, 3, 1> last_w = Eigen::Matrix<double, 3, 1>::Zero();
  if (prop_data.size() > 1){
//    last_w = prop_data.at(prop_data.size() - 2).wm - state->_imu->bias_g();
    const auto& bias_g = state->_imu->bias_g();
    last_w(0) = prop_data.at(prop_data.size() - 2).data->angular_velocity.x - bias_g(0);
    last_w(1) = prop_data.at(prop_data.size() - 2).data->angular_velocity.y - bias_g(1);
    last_w(2) = prop_data.at(prop_data.size() - 2).data->angular_velocity.z - bias_g(2);
  }
  else if (!prop_data.empty()){
//    last_w = prop_data.at(prop_data.size() - 1).wm - state->_imu->bias_g();
    const auto& bias_g = state->_imu->bias_g();
    last_w(0) = prop_data.at(prop_data.size() - 1).data->angular_velocity.x - bias_g(0);
    last_w(1) = prop_data.at(prop_data.size() - 1).data->angular_velocity.y - bias_g(1);
    last_w(2) = prop_data.at(prop_data.size() - 1).data->angular_velocity.z - bias_g(2);
  }
  ...
  printf(RED "STEP 5: " RESET);
  StateHelper::augment_clone(state, last_w);
  printf(RED "STEP 6: " RESET);
}
...
bool Propagator::fast_state_propagate(std::shared_ptr<State> state, double timestamp, Eigen::Matrix<double, 13, 1> &state_plus,
  ...
  // Biases
  Eigen::Vector3d bias_g = cache_state_est.block<3, 1>(10, 0);
  Eigen::Vector3d bias_a = cache_state_est.block<3, 1>(13, 0);

  const Eigen::Matrix<double, 15, 15> MatZero15 = Eigen::Matrix<double, 15, 15>::Zero();
  const Eigen::Matrix<double, 15, 12> MatZero15x12 = Eigen::Matrix<double, 15, 12>::Zero();
  const Eigen::Matrix<double, 12, 12> MatZero12 = Eigen::Matrix<double, 12, 12>::Zero();
...
//    Eigen::Vector3d a_hat = 0.5 * (prop_data.at(i + 1).am + prop_data.at(i).am) - bias_a;
    ...
    w_hat(0) = 0.5 * (prop_data.at(i + 1).data->angular_velocity.x + prop_data.at(i).data->angular_velocity.x) - bias_g(0);
    w_hat(1) = 0.5 * (prop_data.at(i + 1).data->angular_velocity.y + prop_data.at(i).data->angular_velocity.y) - bias_g(1);
    w_hat(2) = 0.5 * (prop_data.at(i + 1).data->angular_velocity.z + prop_data.at(i).data->angular_velocity.z) - bias_g(2);
    Eigen::Vector3d a_hat;
    a_hat(0) = 0.5 * (prop_data.at(i + 1).data->linear_acceleration.x + prop_data.at(i).data->linear_acceleration.x) - bias_a(0);
    a_hat(1) = 0.5 * (prop_data.at(i + 1).data->linear_acceleration.y + prop_data.at(i).data->linear_acceleration.y) - bias_a(1);
    a_hat(2) = 0.5 * (prop_data.at(i + 1).data->linear_acceleration.z + prop_data.at(i).data->linear_acceleration.z) - bias_a(2);

    Eigen::Matrix3d R_Gtoi = quat_2_Rot(cache_state_est.block<4, 1>(0, 0));
    Eigen::Vector3d v_iinG = cache_state_est.block<3, 1>(7, 0);
    Eigen::Vector3d p_iinG = cache_state_est.block<3, 1>(4, 0);

    // State transition and noise matrix
    Eigen::Matrix<double, 15, 15> F = MatZero15;
    F.block<3, 3>(0, 0) = exp_so3(-w_hat * dt);
    F.block<3, 3>(0, 9).noalias() = -exp_so3(-w_hat * dt) * Jr_so3(-w_hat * dt) * dt;
    F.block<3, 3>(9, 9).setIdentity();
    F.block<3, 3>(6, 0).noalias() = -R_Gtoi.transpose() * skew_x(a_hat * dt);
    F.block<3, 3>(6, 6).setIdentity();
    F.block<3, 3>(6, 12) = -R_Gtoi.transpose() * dt;
    F.block<3, 3>(12, 12).setIdentity();
    F.block<3, 3>(3, 0).noalias() = -0.5 * R_Gtoi.transpose() * skew_x(a_hat * dt * dt);
    F.block<3, 3>(3, 6) = Eigen::Matrix3d::Identity() * dt;
    F.block<3, 3>(3, 12) = -0.5 * R_Gtoi.transpose() * dt * dt;
    F.block<3, 3>(3, 3).setIdentity();
    Eigen::Matrix<double, 15, 12> G = MatZero15x12;
    G.block<3, 3>(0, 0) = -exp_so3(-w_hat * dt) * Jr_so3(-w_hat * dt) * dt;
    G.block<3, 3>(6, 3) = -R_Gtoi.transpose() * dt;
    ...
    Eigen::Matrix<double, 15, 15> Qd = MatZero15;
    Eigen::Matrix<double, 12, 12> Qc = MatZero12;
    ...
    cache_state_est.block<4, 1>(0, 0) = rot_2_quat(exp_so3(-w_hat * dt) * R_Gtoi);
    cache_state_est.block<3, 1>(4, 0) = p_iinG + v_iinG * dt + 0.5 * R_Gtoi.transpose() * a_hat * dt * dt - 0.5 * _gravity * dt * dt;
    cache_state_est.block<3, 1>(7, 0) = v_iinG + R_Gtoi.transpose() * a_hat * dt - _gravity * dt;
  }
  ...
  Eigen::Vector4d q_Gtoi = cache_state_est.block<4, 1>(0, 0);
  Eigen::Vector3d v_iinG = cache_state_est.block<3, 1>(7, 0);
  Eigen::Vector3d p_iinG = cache_state_est.block<3, 1>(4, 0);
  state_plus.setZero();
  state_plus.block<4, 1>(0, 0) = q_Gtoi;
  state_plus.block<3, 1>(4, 0) = p_iinG;
  state_plus.block<3, 1>(7, 0) = quat_2_Rot(q_Gtoi) * v_iinG;
//  state_plus.block(10, 0, 3, 1) = 0.5 * (prop_data.at(prop_data.size() - 1).wm + prop_data.at(prop_data.size() - 2).wm) - bias_g;
  Eigen::Vector3d block_tmp;
  block_tmp(0) = 0.5 * (prop_data.at(prop_data.size() - 1).data->angular_velocity.x + prop_data.at(prop_data.size() - 2).data->angular_velocity.x) - bias_g(0);
  block_tmp(1) = 0.5 * (prop_data.at(prop_data.size() - 1).data->angular_velocity.y + prop_data.at(prop_data.size() - 2).data->angular_velocity.y) - bias_g(1);
  block_tmp(2) = 0.5 * (prop_data.at(prop_data.size() - 1).data->angular_velocity.z + prop_data.at(prop_data.size() - 2).data->angular_velocity.z) - bias_g(2);
  state_plus.block<3, 1>(10, 0) = block_tmp;
  ...
  covariance.setZero();
  Eigen::Matrix<double, 15, 15> Phi = I15;
  Phi.block<3, 3>(6, 6) = quat_2_Rot(q_Gtoi);
  Eigen::MatrixXd covariance_tmp = Phi * cache_state_covariance * Phi.transpose();
  covariance.block<9, 9>(0, 0) = covariance_tmp.block(0, 0, 9, 9);
  double dt = prop_data.at(prop_data.size() - 1).timestamp - prop_data.at(prop_data.size() - 2).timestamp;
  covariance.block<3, 3>(9, 9) = _noises.sigma_w_2 / dt * Eigen::Matrix3d::Identity();
  return true;
}
...
std::vector<ov_core::ImuData> Propagator::select_imu_readings(const std::vector<ov_core::ImuData> &imu_data, double time0, double time1,
  ...
  prop_data.reserve(imu_data.size());
...
//  Eigen::Matrix<double, 3, 1> a_hat2 = data_plus.am - state->_imu->bias_a();
  const auto& bias_g = state->_imu->bias_g();
  const auto& bias_a = state->_imu->bias_a();
  Eigen::Matrix<double, 3, 1> w_hat;
  w_hat(0) = data_minus.data->angular_velocity.x - bias_g(0);
  w_hat(1) = data_minus.data->angular_velocity.y - bias_g(1);
  w_hat(2) = data_minus.data->angular_velocity.z - bias_g(2);
  Eigen::Matrix<double, 3, 1> a_hat;
  a_hat(0) = data_minus.data->linear_acceleration.x - bias_a(0);
  a_hat(1) = data_minus.data->linear_acceleration.y - bias_a(1);
  a_hat(2) = data_minus.data->linear_acceleration.z - bias_a(2);
  Eigen::Matrix<double, 3, 1> w_hat2;
  w_hat2(0) = data_plus.data->angular_velocity.x - bias_g(0);
  w_hat2(1) = data_plus.data->angular_velocity.y - bias_g(1);
  w_hat2(2) = data_plus.data->angular_velocity.z - bias_g(2);
  Eigen::Matrix<double, 3, 1> a_hat2;
  a_hat2(0) = data_plus.data->linear_acceleration.x - bias_a(0);
  a_hat2(1) = data_plus.data->linear_acceleration.y - bias_a(1);
  a_hat2(2) = data_plus.data->linear_acceleration.z - bias_a(2);
