...
public:
   ...
   * @param state Pointer to state
   */
  static void marginalize_keyframes(const std::shared_ptr<ov_core::FeatureDatabase>& databse, std::shared_ptr<State> state);

  /**
   * @brief Marginalize bad SLAM features
   * @param state Pointer to state
   */
  static void marginalize_map(std::shared_ptr<State> state);
