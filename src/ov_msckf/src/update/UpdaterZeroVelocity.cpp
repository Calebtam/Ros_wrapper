...
//    Eigen::Vector3d a_hat = imu_recent.at(i).am - state->_imu->bias_a();
    ...
    a_hat(0) = imu_recent.at(i).data->linear_acceleration.x - bias_a(0);
    a_hat(1) = imu_recent.at(i).data->linear_acceleration.y - bias_a(1);
    a_hat(2) = imu_recent.at(i).data->linear_acceleration.z - bias_a(2);
...
//    res.block(6 * i + 0, 0, 3, 1) = -w_omega * (imu_recent.at(i).wm - state->_imu->bias_g());
    ...
    block_tmp(0) = -w_omega * (imu_recent.at(i).data->angular_velocity.x - bias_g(0));
    block_tmp(1) = -w_omega * (imu_recent.at(i).data->angular_velocity.y - bias_g(1));
    block_tmp(2) = -w_omega * (imu_recent.at(i).data->angular_velocity.z - bias_g(2));
    res.block(6 * i + 0, 0, 3, 1) = block_tmp;
    if (!integrated_accel_constraint) {
