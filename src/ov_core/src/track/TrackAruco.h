...
public:
   ...
   */
  explicit TrackAruco(std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras, int numaruco, std::shared_ptr<ov_msckf::VioManagerOptions>& params)
      : TrackBase(cameras, 0, numaruco, params), max_tag_id(numaruco), do_downsizing(params->downsize_aruco) {
#if ENABLE_ARUCO_TAGS
