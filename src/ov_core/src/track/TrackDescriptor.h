...
public:
   ...
   */
  explicit TrackDescriptor(std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras, int numfeats, int numaruco, std::shared_ptr<ov_msckf::VioManagerOptions>& params)
      : TrackBase(cameras, numfeats, numaruco, params), threshold(params->fast_threshold), grid_x(params->grid_x), grid_y(params->grid_y),
        min_px_dist(params->min_px_dist), knn_ratio(params->knn_ratio) {}
