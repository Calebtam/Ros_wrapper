...
void TrackDescriptor::feed_monocular(const CameraData &message, size_t msg_id) {
  ...
  mask = params_ptr->masks.at(msg_id);
...
void TrackDescriptor::feed_stereo(const CameraData &message, size_t msg_id_left, size_t msg_id_right) {
  ...
  if (histogram_method == HistogramMethod::HISTOGRAM) {
    cv::equalizeHist(message.images[msg_id_left], img_left);
    cv::equalizeHist(message.images[msg_id_right], img_right);
  } else if (histogram_method == HistogramMethod::CLAHE) {
    double eq_clip_limit = 10.0;
    cv::Size eq_win_size = cv::Size(8, 8);
    cv::Ptr<cv::CLAHE> clahe = cv::createCLAHE(eq_clip_limit, eq_win_size);
    clahe->apply(message.images[msg_id_left], img_left);
    clahe->apply(message.images[msg_id_right], img_right);
  } else {
    img_left = message.images[msg_id_left];
    img_right = message.images[msg_id_right];
  }
  mask_left = params_ptr->masks.at(msg_id_left);
  mask_right = params_ptr->masks.at(msg_id_right);
