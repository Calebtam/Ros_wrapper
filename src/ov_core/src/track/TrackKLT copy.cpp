...
void TrackKLT::feed_stereo(const CameraData &message, size_t msg_id_left, size_t msg_id_right) {
  ...
  for (size_t i = 0; i < pts_left_new.size(); i++) {
    ...
    if (mask_ll[i] && found_right && mask_rr[index_right]) {
      ...
      num_of_points++;
      cv::circle(kp_im_show1, pts_left_new.at(i).pt, 3 * fontsize, cv::Scalar(0, 210,   0), cv::FILLED); //BGR
      cv::line(kp_im_show1, pts_right_new.at(index_right).pt, pts_left_new.at(i).pt, cv::Scalar(0, 128, 0), 1*fontsize);
      cv::line(kp_im_show1, pts_left_new.at(i).pt, pts_left_old.at(i).pt, cv::Scalar(0, 245, 245), 1*fontsize);

      cv::circle(kp_im_show2, pts_right_new.at(index_right).pt, 3 * fontsize, cv::Scalar(0, 210, 0), cv::FILLED); //BGR
      cv::line(kp_im_show2, pts_left_new.at(i).pt, pts_right_new.at(index_right).pt, cv::Scalar(0, 128, 0), 1*fontsize);
      cv::line(kp_im_show2, pts_right_new.at(index_right).pt, pts_right_old.at(index_right).pt, cv::Scalar(0, 245, 245), 1*fontsize);
    ...
    else if (mask_ll[i]) 
    {
      good_left.push_back(pts_left_new.at(i));
      good_ids_left.push_back(ids_left_old.at(i));
      cv::circle(kp_im_show1, pts_left_new.at(i).pt, 3 * fontsize, cv::Scalar(0, 255, 255), cv::FILLED); //BGR
      cv::line(kp_im_show1, pts_left_new.at(i).pt, pts_left_old.at(i).pt, cv::Scalar(0, 255, 255), 1*fontsize);
    ...
    if(!mask_ll[i])
    {
      ...
      buf << ids_left_old[i];
      cv::putText(kp_im_show1, buf.str(), cv::Point2f(pts_left_old.at(i).pt.x-20,pts_left_old.at(i).pt.y-10), cv::FONT_HERSHEY_SIMPLEX, 0.4*fontsize, cv::Scalar(0, 0, 255), 1*fontsize);
      cv::circle(kp_im_show1, pts_left_old.at(i).pt, 3 * fontsize, cv::Scalar(0, 0, 255), cv::FILLED); //BGR
      cv::line(kp_im_show1, pts_left_new.at(i).pt, pts_left_old.at(i).pt, cv::Scalar(0, 0, 245), 1*fontsize);
    }
    if(!mask_rr[index_right])
    {
      std::stringstream buf;
      buf << ids_right_old[index_right];
      cv::putText(kp_im_show2, buf.str(), cv::Point2f(pts_right_old.at(index_right).pt.x-20,pts_right_old.at(index_right).pt.y-10), cv::FONT_HERSHEY_SIMPLEX, 0.4*fontsize, cv::Scalar(0, 0, 255), 1*fontsize);
      cv::circle(kp_im_show2, pts_right_old.at(index_right).pt, 3 * fontsize, cv::Scalar(0, 0, 255), cv::FILLED); //BGR
      cv::line(kp_im_show2, pts_right_new.at(index_right).pt, pts_right_old.at(index_right).pt, cv::Scalar(0, 0, 245), 1*fontsize);
    }
  }
  ...
  for (size_t i = 0; i < pts_right_new.size(); i++) {
    ...
    if (mask_rr[i] && !added_already) {
      ...
      cv::circle(kp_im_show2, pts_right_new.at(i).pt, 3 * fontsize, cv::Scalar(0, 255, 255), cv::FILLED); //BGR
      cv::line(kp_im_show2, pts_right_new.at(i).pt, pts_right_old.at(i).pt, cv::Scalar(0, 255, 255), 1*fontsize);
    ...
    if(!mask_rr[i])
    {
      std::stringstream buf;
      buf << ids_right_old[i];
      cv::putText(kp_im_show1, buf.str(), cv::Point2f(pts_right_old.at(i).pt.x-20,pts_right_old.at(i).pt.y-10), cv::FONT_HERSHEY_SIMPLEX, 0.4*fontsize, cv::Scalar(0, 0, 255), 1*fontsize);
      cv::circle(kp_im_show1, pts_right_old.at(i).pt, 3 * fontsize, cv::Scalar(0, 0, 255), cv::FILLED); //BGR
    }
  }
...
void TrackKLT::feed_stereo(const CameraData &message, size_t msg_id_left, size_t msg_id_right, int a) {
