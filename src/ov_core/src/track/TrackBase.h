...
public:
   ...
   */
  enum HistogramMethod { NONE = 0, HISTOGRAM, CLAHE };
   ...
   */
  TrackBase(std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras, int numfeats, int numaruco, std::shared_ptr<ov_msckf::VioManagerOptions>& params);
...
protected:
  ...
  std::shared_ptr<ov_msckf::VioManagerOptions> params_ptr;
