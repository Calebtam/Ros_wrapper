...
TrackBase::TrackBase(std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras, int numfeats, int numaruco, std::shared_ptr<ov_msckf::VioManagerOptions>& params)
    : camera_calib(cameras), database(new FeatureDatabase()), num_features(numfeats), use_stereo(params->use_stereo), histogram_method(params->histogram_method) {
  params_ptr = params;
  ...
  for (auto const &pair : img_last_cache) {
    ...
    for (size_t i = 0; i < ids_last_cache[pair.first].size(); i++) {
      ...
      if (std::find(highlighted.begin(), highlighted.end(), ids_last_cache[pair.first].at(i)) != highlighted.end()) {
        ...
        cv::circle(img_temp, pt_c, (is_small) ? 2 : 4, cv::Scalar(0, 255, 0), cv::FILLED);
      }
...
//        cv::circle(img_temp, pt_c, (is_small) ? 2 : 4, cv::Scalar(color_r, color_g, color_b), cv::FILLED);
        cv::circle(img_temp, pt_c, (is_small) ? 2 : 4, cv::Scalar(255, 0, 0), cv::FILLED);
