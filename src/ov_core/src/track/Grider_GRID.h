...
//    auto T1 = boost::posix_time::microsec_clock::local_time();
    ...
    cv::TermCriteria term_crit = cv::TermCriteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS, 5, 0.01);//tune: default is 20, 0.001
...
//    auto T2 = boost::posix_time::microsec_clock::local_time();
    ...
    cv::cornerSubPix(img, pts_refined, win_size, zero_zone, term_crit);//time cost! 70%
...
//    PRINT_ALL("[grid]: %.4f seconds to calculate1\n", (T1 - T0).total_microseconds() * 1e-6);
//    PRINT_ALL("[grid]: %.4f seconds to calculate2\n", (T2 - T1).total_microseconds() * 1e-6);
//    PRINT_ALL("[grid]: %.4f seconds to calculate3\n", (T3 - T2).total_microseconds() * 1e-6);
//    PRINT_ALL("[grid]: %.4f seconds to calculate4\n", (T3 - T0).total_microseconds() * 1e-6);
  }
};
